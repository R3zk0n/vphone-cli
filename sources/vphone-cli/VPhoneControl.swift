...
class VPhoneControl {
    ...
    func keychainGetValue(
        itemClass: String, account: String = "", service: String = "",
        accessGroup: String = "", server: String = "", rowid: Int = 0
    ) async throws -> KeychainValueResult {
        var req: [String: Any] = [
            "t": "keychain_get", "class": itemClass,
            "account": account, "service": service,
            "accessGroup": accessGroup, "server": server,
        ]
        if rowid > 0 { req["rowid"] = rowid }
        let (resp, _) = try await sendRequest(req)
        let ok = resp["ok"] as? Bool ?? false
        if !ok {
            let msg = resp["msg"] as? String ?? "unknown error"
            throw ControlError.protocolError("keychain_get: \(msg)")
        }
        return KeychainValueResult(
            value: resp["value"] as? String ?? "",
            valueEncoding: resp["valueEncoding"] as? String ?? "",
            valueType: resp["valueType"] as? String,
            valueSize: (resp["valueSize"] as? NSNumber)?.intValue ?? 0,
            decrypted: resp["decrypted"] as? Bool ?? false,
            source: resp["source"] as? String,
            diagnostics: resp["diag"] as? [String] ?? []
        )
    }
